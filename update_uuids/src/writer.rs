...
use crate::yaml::{AdType, Category, UuidData};
...
pub const {}: BluetoothUuid16 = BluetoothUuid16::new(0x{:x});",
        ...
        write_rust_file(&mut file, &module_name, tokens, commit_hash, true)?;
    }
    Ok(())
}
...
fn write_rust_file(
    ...
    writeln!(file, "{}", tokens)?; // write the file contents
    Ok(())
}
...
pub fn update_ad_types(output_folder: &Path, input: &[AdType], commit_hash: &str) -> Result<(), Box<dyn Error>> {
    let (module_name, mut file) = setup_rust_file("ad_types", output_folder.into())?;

    let constants: Vec<_> = input
        .iter()
        .map(|ad_type| {
            format!(
                "/// Bluetooth {} Common Data Type.
///
/// `0x{:02x}`
/// See Bluetooth {}.
pub const {}: u8 = 0x{:x};",
                ...
                ad_type.value,
                ad_type.reference,
                screaming_snake_case(&ad_type.name),
                ad_type.value,
            )
        })
        .collect();
    ...
    write_rust_file(&mut file, &module_name, tokens, commit_hash, false)?;
